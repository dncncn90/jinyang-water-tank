...
export async function POST(request: Request) {
    ...
    try {
        ...
        try {
            ...
            if (discordWebhookUrl && !paymentMethod) { // 결제 수단이 없을 때만(상담 신청) 알림 발송
                const itemText = items.map((i: any) => `- ${i.name} (${i.options || '기본'}) : ${i.quantity}개`).join('\n');
                const discordMessage = {
                    username: "진양건재 실시간 알리미 🚨",
                    embeds: [{
                        title: "📞 [결제 전 상담] 주문 상담 접수됨",
                        color: 15105570, // 주황색 계열
                        fields: [
                            { name: "주문번호", value: orderUuid, inline: true },
                            { name: "주문총액", value: `${totalAmount.toLocaleString()}원`, inline: true },
                            { name: "고객명", value: name, inline: true },
